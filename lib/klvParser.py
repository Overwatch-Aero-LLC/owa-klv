...
from lib.misb0601_decoder import decode_misb0601_item, misb0601_key_names
...
if __name__ == "__main__":
    ...
    with open('../testfiles/test.bin', 'rb') as f:
        rawBinary = f.read()
    ...
    # Collect all unique keys while preserving the order as they appear in each packet.
    all_keys = []
    ...
    with open(csv_filename, mode='a', newline='') as csvfile:
        ...
        # Write headers (Packet number and all unique keys) if the file does not exist
        if not file_exists:
            writer.writerow(["Packet"] + all_keys)

        # Write data for each packet
        for packet_num, packet_data in parsed.items():
            # Build a row with the packet number, then each key's value in the same header order,
            # using None if a key is missing.
